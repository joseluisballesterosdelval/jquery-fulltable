...
(function($) {
    ...
    $.fn.FullTable = function() {
		...
		var addEditionControl = function(tr) {
			if (!options.editable) return this;
			if (!$(tr).is("tr")) return this;
			...
			if ($(tr).parent().is("tbody")) {
				var tr_index = $(tr).data("fulltable-index");
		...
		var methods = {
			...
			'filter':function() {
				...
				$(table).find("thead th input, thead th select").each(function (i, e) {
			...
			'addRow':function() {
				...
				rows[row_index] = row;
				row["__creating"] = true;
				row["__dom"] = $("<tr/>").data("fulltable-index", row_index);
				...
				addEditionControl($(row["__dom"]));
				showRowForm(row);
				return this;
			},
			'editRow':function(row_index) {
				if (!options.editable) return this;
			...
			'getData':function() {
				...
				for (var row in rows) {
					row = rows[row];
					var resultRow = {};
					if (row["__removed"] == true) continue;
		...
		$(table).find("thead th").each(function(th_index, th) {
			...
			$(th).children("input, select").remove();
		...
		addEditionControl($(table).find("thead tr"));

		$(table).find("tbody tr").each(function(tr_index, tr) {
			$(tr).data("fulltable-index", tr_index);
			rows[tr_index] = {};
			var row = rows[tr_index];
			...
			// Appending of edition controls
			addEditionControl($(tr));
